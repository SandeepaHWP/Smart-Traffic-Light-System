...
# Offset for line crossing detection
offset = 6
counter = 0
detect = []
...
# Function to calculate center of bounding box
...
while True:
    ...
    # Line position: a little above the bottom
    count_line_position = frame_height - 50  # 50 pixels above bottom
    ...
    # Draw horizontal half-line (left to middle)
    ...
    for (cx, cy) in detect:
        if (count_line_position - offset) < cy < (count_line_position + offset):
            if cx < frame_width // 2:  # Only count if inside half-width area
                counter += 1
                detect.remove((cx, cy))
